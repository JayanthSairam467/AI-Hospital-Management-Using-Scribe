...
import { UserRole, hasAccess } from "../config/roles";
    ...
    >
      <div className="space-y-1">
        ...
        {navItems.map((item) => {
          ...
          const isAllowed = item.id === "dashboard" || hasAccess(userRole, item.id);
          ...
          return (
            <button
              ...
              title={collapsed ? item.label : !isAllowed ? `🔒 Requires elevated access` : undefined}
              ...
              <div className={`flex items-center ${collapsed ? "justify-center" : "space-x-3"} truncate`}>
                ...
                {!collapsed && (
                  <div className="truncate">
                    <div className={`font-semibold ${!isAllowed ? "text-slate-400" : isActive ? "text-white" : "text-slate-800"}`}>
                      {item.label}
                    </div>
                    <div className={`text-[10px] truncate ${!isAllowed ? "text-slate-300" : isActive ? "text-blue-100" : "text-slate-400"}`}>
                      {item.description}
                    </div>
                  </div>
                )}
