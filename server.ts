...
// API: Auto-Reorder Supplier Email (stock < 10% of maxUnits)
app.post("/api/inventory/auto-reorder", async (req, res) => {
  try {
    const { medicine, stockPercent } = req.body;

    if (!medicine || !medicine.name) {
      return res.status(400).json({ error: "Medicine data is required" });
    }

    const SUPPLIER_EMAIL = "jayanthsairam8418@gmail.com";
    const REORDER_THRESHOLD = 10; // 10% of max capacity

    // Calculate reorder quantity (bring back to 80% of max)
    const reorderQty = Math.ceil(medicine.maxUnits * 0.8) - medicine.stockUnits;
    const urgency = stockPercent <= 5 ? "CRITICAL" : stockPercent <= 10 ? "URGENT" : "STANDARD";

    // Configure email transport
    // In production, use SMTP_USER and SMTP_PASS from .env
    const smtpUser = process.env.SMTP_USER;
    const smtpPass = process.env.SMTP_PASS;

    if (!smtpUser || !smtpPass) {
      // Log the reorder and return success even without email (for hackathon demo)
      console.log(`[AUTO-REORDER] ${urgency} reorder triggered for ${medicine.name}`);
      console.log(`  → Stock: ${medicine.stockUnits}/${medicine.maxUnits} (${stockPercent}%)`);
      console.log(`  → Reorder Qty: ${reorderQty} ${medicine.unitType}`);
      console.log(`  → Supplier Email: ${SUPPLIER_EMAIL}`);

      return res.json({
        success: true,
        message: `Auto-reorder logged for ${medicine.name}. Email sending requires SMTP credentials in .env`,
        reorderDetails: {
          medicine: medicine.name,
          currentStock: medicine.stockUnits,
          maxCapacity: medicine.maxUnits,
          stockPercent,
    ...
    const htmlEmail = `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
        <div style="background: ${urgency === 'CRITICAL' ? '#dc2626' : urgency === 'URGENT' ? '#f59e0b' : '#2563eb'}; padding: 20px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 20px;">⚕️ OmniScribe Health — Automated Purchase Order</h1>
          <p style="color: rgba(255,255,255,0.85); margin: 4px 0 0; font-size: 13px;">Priority: ${urgency}</p>
        </div>
        <div style="padding: 24px;">
          <h2 style="color: #1e293b; margin: 0 0 16px;">Medication Reorder Request</h2>
          <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Medication</td>
              <td style="padding: 8px; font-weight: bold; color: #0f172a;">${medicine.name} (${medicine.dosage})</td>
            </tr>
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Category</td>
              <td style="padding: 8px; color: #0f172a;">${medicine.category}</td>
            </tr>
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Batch #</td>
              <td style="padding: 8px; font-family: monospace; color: #0f172a;">${medicine.batchNumber}</td>
            </tr>
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Current Stock</td>
              <td style="padding: 8px; color: ${urgency === 'CRITICAL' ? '#dc2626' : '#f59e0b'}; font-weight: bold;">${medicine.stockUnits} / ${medicine.maxUnits} ${medicine.unitType} (${stockPercent}%)</td>
            </tr>
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Daily Consumption</td>
              <td style="padding: 8px; color: #0f172a;">~${medicine.dailyConsumption} ${medicine.unitType}/day</td>
            </tr>
            <tr style="border-bottom: 1px solid #e2e8f0;">
              <td style="padding: 8px; color: #64748b;">Days Until Stockout</td>
              <td style="padding: 8px; color: #dc2626; font-weight: bold;">~${Math.floor(medicine.stockUnits / medicine.dailyConsumption)} days</td>
            </tr>
            <tr style="background: #f0f9ff;">
              <td style="padding: 12px; color: #1e40af; font-weight: bold;">🔄 ORDER QUANTITY</td>
              <td style="padding: 12px; color: #1e40af; font-weight: bold; font-size: 18px;">${reorderQty} ${medicine.unitType}</td>
            </tr>
          </table>
          <div style="margin-top: 20px; padding: 12px; background: #fefce8; border: 1px solid #fde68a; border-radius: 8px; font-size: 12px; color: #92400e;">
            ⚠️ This is an automated order from OmniScribe Health Pharmacy Management System. Please confirm availability and estimated delivery time.
          </div>
          <p style="margin-top: 16px; font-size: 11px; color: #94a3b8; text-align: center;">
            Metro General Hospital • Central Pharmacy Unit • Generated ${new Date().toISOString()}
          </p>
        </div>
      </div>
    `;
    ...
    await transporter.sendMail({
      from: `"OmniScribe Pharmacy" <${smtpUser}>`,
      to: SUPPLIER_EMAIL,
      subject: `[${urgency}] Auto-Reorder: ${medicine.name} — Stock at ${stockPercent}%`,
      html: htmlEmail,
    });

    console.log(`[AUTO-REORDER] Email sent to ${SUPPLIER_EMAIL} for ${medicine.name}`);

    return res.json({
      success: true,
      message: `Purchase order email sent to ${SUPPLIER_EMAIL}`,
      reorderDetails: {
        medicine: medicine.name,
        currentStock: medicine.stockUnits,
        maxCapacity: medicine.maxUnits,
    ...
    res.status(500).json({ error: error.message || "Failed to process auto-reorder" });
  }
});
